"""
Sastavljanje i rastavljanje PGP poruke (.msg fajla).
Redosled pri slanju:  potpis -> kompresija -> sifrovanje -> radix-64
Redosled pri prijemu: radix-64^-1 -> desifrovanje -> dekompresija -> razdvajanje -> verifikacija

FORMAT (pre radix-64):
  [flags 1B]
  -- ako je ENCRYPTED:  put(recipient_key_id) | algo(1B) | put(iv) | put(enc_Ks) | put(ciphertext)
  -- inace:             put(inner)
  inner (posle desifrovanja/dekompresije):
     put(sig_comp) | put(msg_comp)
     sig_comp = put(sender_key_id) | put(timestamp) | put(signature)   (prazno ako nije potpisano)
     msg_comp = put(filename) | put(timestamp) | put(data)
Radix-64 se prepoznaje po ASCII omotacu (ARMOR), ne po flegu.
"""
import zlib
import base64
import time
...
# --- pomocne: polje = 4 bajta duzine + sadrzaj ---
def _put(block):
...
def _dearmor(raw):
    """Ako je poruka u radix-64 omotacu, skini ga i vrati (bajtovi, True)."""
...
def build_message(data, filename="poruka.txt", sign_priv=None, sender_key_id=None,
    ...
    # radix-64 (poslednji korak)
    if radix64:
        out = ARMOR_HEADER + base64.b64encode(out) + ARMOR_FOOTER
    return out
...
def read_recipient_key_id(raw):
    """Procitaj ciji je privatni kljuc potreban, BEZ desifrovanja."""
    raw, _ = _dearmor(raw)
    flags = raw[0]
...
def parse_message(raw, private_key=None):
    """Rastavi poruku. Verifikaciju potpisa radi pozivalac
    (koristeci vraceni sender_key_id + signature + data)."""
    raw, was_radix64 = _dearmor(raw)          # 1) skini radix-64 ako ga ima
    flags = raw[0]                            # 2) procitaj bajt flegova
    offset = 1
    ...
    # 4) dekompresija (ako treba)
    if flags & FLAG_COMPRESSED:
        inner = zlib.decompress(inner)

    # 5) razdvoj dve komponente iz inner-a
    sig_comp, p = _get(inner, 0)
    msg_comp, p = _get(inner, p)

    # 6) iz sig_comp izvuci polja (samo ako je potpisano)
